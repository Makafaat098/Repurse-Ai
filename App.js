...
export default function App() {
  ...
  const generateOne = async (formatId) => {
    ...
    try {
      ...
      setResults((prev) => ({ ...prev, [formatId]: "⚠️ Error. Please try again." }));
    }
    ...
    <div style={{
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <span style={{
              ...
              fontWeight: "bold",
              letterSpacing: "-0.5px",
              background: "linear-gradient(135deg, #f4a261 30%, #e63946 100%)",
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",
            }}>RepurseAI</span>
          </div>
          <div style={{ fontSize: "12px", color: "#666", marginTop: "4px", fontFamily: "monospace" }}>
            1 post → 10 formats · Powered by Claude AI
          </div>
        </div>
        <div style={{
          background: "#1a1a2e",
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ marginBottom: "36px" }}>
          ...
          />
          <div style={{ textAlign: "right", fontSize: "12px", color: "#444", marginTop: "6px", fontFamily: "monospace" }}>
            ...
            {FORMATS.map((f) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSel ? `${f.color}22` : "#0f0f1a",
                    border: `1px solid ${isSel ? f.color : "#2a2a3e"}`,
                    borderRadius: "10px",
                    padding: "12px 14px",
                    cursor: "pointer",
                    textAlign: "left",
                    color: isSel ? "#f0ede8" : "#666",
                    transition: "all 0.2s",
                    position: "relative",
                  }}
                >
        ...
        <button
          onClick={generateAll}
          disabled={!input.trim() || selected.length === 0 || generating}
          style={{
            width: "100%",
            padding: "18px",
            ...
            fontSize: "16px",
            fontWeight: "bold",
            cursor: generating || !input.trim() || selected.length === 0 ? "not-allowed" : "pointer",
            letterSpacing: "1px",
            fontFamily: "monospace",
            transition: "opacity 0.2s",
            opacity: !input.trim() || selected.length === 0 ? 0.4 : 1,
            marginBottom: "40px",
          }}
        >
          {generating
            ? `⟳ Generating ${loadingFormats.length} formats...`
        ...
        {doneFormats.length > 0 && (
          <div>
              ...
              {doneFormats.map((id) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: activeTab === id ? `${f.color}33` : "transparent",
                      border: `1px solid ${activeTab === id ? f.color : "#2a2a3e"}`,
                      color: activeTab === id ? "#f0ede8" : "#666",
                      padding: "7px 16px",
                      borderRadius: "20px",
                      cursor: "pointer",
                      fontSize: "12px",
            ...
            {activeTab && results[activeTab] && (
              <div style={{
                background: "#0f0f1a",
                border: `1px solid ${FORMATS.find((f) => f.id === activeTab)?.color || "#2a2a3e"}`,
                borderRadius: "12px",
                padding: "28px",
                position: "relative",
              }}>
                <button
                  onClick={() => copyText(activeTab, results[activeTab])}
                  style={{
                    position: "absolute",
                    top: "16px",
                    right: "16px",
                    background: copied === activeTab ? "#2a9d8f22" : "#1a1a2e",
                    border: `1px solid ${copied === activeTab ? "#2a9d8f" : "#2a2a3e"}`,
                    color: copied === activeTab ? "#2a9d8f" : "#888",
                    padding: "6px 14px",
                    borderRadius: "20px",
                    cursor: "pointer",
                    fontSize: "12px",
        ...
        {Object.keys(results).length === 0 && !generating && (
          <div style={{
            ...
            <div style={{ fontFamily: "monospace", fontSize: "14px" }}>
              Content paste karein, formats choose karein, generate dabao
            </div>
          </div>
        )}
      </div>
